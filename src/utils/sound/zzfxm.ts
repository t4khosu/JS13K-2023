...
export const zzfxP = (...t: (string | any[])[]) => {
    let e = zzfxX.createBufferSource(), f = zzfxX.createBuffer(t.length, t[0].length, zzfxR);
    // @ts-ignore
    t.map((d, i) => f.getChannelData(i).set(d)), e.loop = true, e.buffer = f, e.connect(zzfxX.destination), e.start();
    return e
}

// zzfxG() - the sound generator -- returns an array of sample data
export const zzfxG = (q = 1, k = .05, c = 220, e = 0, t = 0, u = .1, r = 0, F = 1, v = 0, z = 0, w = 0, A = 0, l = 0, B = 0, x = 0, G = 0, d = 0, y = 1, m = 0, C = 0) => {
    let b = 2 * Math.PI, H = v *= 500 * b / zzfxR ** 2, I = (0 < x ? 1 : -1) * b / 4,
        D = c *= (1 + 2 * k * Math.random() - k) * b / zzfxR, Z = [], g = 0, E = 0, a = 0, n = 1, J = 0, K = 0, f = 0,
        p, h;
    e = 99 + zzfxR * e;
    m *= zzfxR;
    t *= zzfxR;
...
export const zzfxM = (instruments: number[][], patterns: number[][][], sequence: number[], BPM: number) => {
